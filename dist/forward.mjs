...
// [LOG] إشعار تيليجرام للمطوّر عند نجاح/فشل التحويل
function _fwLog(text) {
  if (!_bot) return;
  try { _bot.sendMessage(DEV_ID, `🔄 *نظام التحويل*\n\n${text}`, { parse_mode: "Markdown" }).catch(() => {}); }
  catch {}
}
...
async function _fetchAndShowNewsletter(uid, type, key, displayFn) {
  ...
  try {
    ...
    const name = (typeof meta.name === "string" ? meta.name : meta.id) || meta.id;
...
export async function applyForwardRules(sock, msg) {
  try {
    ...
    // [LOG] كل رسالة قناة تُسجَّل (حتى بدون قاعدة مطابقة)
    const isNewsletter = fromJid.endsWith("@newsletter");
    if (isNewsletter) {
      const matched = rules.some((r) => r.sources.includes(fromJid));
      if (!matched) {
        // رسالة من قناة لا توجد لها قاعدة — لا نُرسل إشعاراً لتجنب الإزعاج
        return;
      }
    }
      ...
      try {
        ...
        if (rule.noForward) {
          if (m.conversation || m.extendedTextMessage) {
            ...
            if (buf) await sock.sendMessage(destJid, { image: buf, caption: text });
            else await sock.sendMessage(destJid, { forward: msg });
          } else if (m.videoMessage) {
            const buf = await _downloadMedia(sock, msg);
            if (buf) await sock.sendMessage(destJid, { video: buf, caption: text, mimetype: m.videoMessage.mimetype });
            else await sock.sendMessage(destJid, { forward: msg });
          } else if (m.audioMessage) {
            const buf = await _downloadMedia(sock, msg);
            if (buf) await sock.sendMessage(destJid, { audio: buf, mimetype: m.audioMessage.mimetype || "audio/mp4", ptt: m.audioMessage.ptt });
            else await sock.sendMessage(destJid, { forward: msg });
          } else if (m.stickerMessage) {
            const buf = await _downloadMedia(sock, msg);
            if (buf) await sock.sendMessage(destJid, { sticker: buf });
            else await sock.sendMessage(destJid, { forward: msg });
          } else if (m.documentMessage) {
            const buf = await _downloadMedia(sock, msg);
            if (buf) await sock.sendMessage(destJid, { document: buf, mimetype: m.documentMessage.mimetype, fileName: m.documentMessage.fileName, caption: text });
            else await sock.sendMessage(destJid, { forward: msg });
          } else {
...
export function autoDetectSource(msg) {
  try {
    ...
    const chats = loadChats();
    const list = isChannel ? chats.channels : chats.groups;
    if (!list.find((c) => c.id === jid)) {
      list.push({ id: jid, name: (isChannel ? "📺 قناة " : "👥 ") + jid.split("@")[0] });
      saveChats(chats);
    }
  } catch { /* silent */ }
}
...
function chatListKb(items, page, prefix, selected = [], multi = true, backCb = "fw_menu") {
  ...
  // Pagination
  const nav = [];
  if (page > 0) nav.push({ text: "◀️ السابق", callback_data: prefix + "p" + (page - 1) });
  nav.push({ text: `📄 ${page + 1}/${total || 1}`, callback_data: "noop" });
  if (page < total - 1) nav.push({ text: "التالي ▶️", callback_data: prefix + "p" + (page + 1) });
  rows.push(nav);
  ...
  if (multi && selected.length > 0) {
    rows.push([{ text: `✅ تأكيد الاختيار (${selected.length})`, callback_data: prefix + "ok" }]);
  }
...
export async function handleForwardCallback(query) {
  ...
  if (data === "fw_chpic") {
    ...
    if (!_sock) { ans("⚠️ واتساب غير متصل"); return true; }
    try {
      const url = await _sock.profilePictureUrl(result.id, "image");
      await _bot.sendPhoto(chatId, url, { caption: `🖼️ بروفايل: ${result.name}` }).catch(() => {});
    } catch {
      await send("❌ *تعذّر تنزيل البروفايل*\n\nقد لا تملك هذه القناة صورة.");
    }
    return true;
  }
  ...
  // ── إضافة قاعدة — اختيار المصدر ──────────────────────────────
  if (data === "fw_add") {
    ans();
    ss(uid, { step: "src_type", sources: [], srcIdxMap: [], srcPage: 0, dstPage: 0, blockLinks: false, noForward: false });
  ...
  if (data === "fw_src_ch" || data === "fw_src_gr") {
    ...
    if (!items.length && !isCh) {
      ...
      chats.groups = fetched; saveChats(chats); items = fetched;
    }
  ...
  if (data === "fw_sch_ref" || data === "fw_sgr_ref") {
    ...
    if (!_sock) { ans("⚠️ واتساب غير متصل — اربط الرقم أولاً"); return true; }
    ans("🔄 جاري التحديث...");
    const chats = loadChats();
    let items;
    if (!isCh) { items = await fetchGroups(); chats.groups = items; saveChats(chats); }
  ...
  if (data === "fw_sch_ok" || data === "fw_sgr_ok") {
    ans();
    const s = gs(uid);
    if (!s.sources?.length) { ans("⚠️ اختر مصدراً واحداً على الأقل"); return true; }
    ss(uid, { step: "dst_type" });
    await edit(`➕ *إضافة قاعدة*\n\n✅ تم اختيار ${s.sources.length} مصدر\n\nاختر *نوع الوجهة* — إلى أين تُرسل الرسائل؟`, dstTypeKb());
    return true;
  }
  ...
  if (data === "fw_dst_ch" || data === "fw_dst_gr") {
    ...
    const chats = loadChats();
    let items = isCh ? chats.channels : chats.groups;
    if (!items.length && !isCh) { items = await fetchGroups(); chats.groups = items; saveChats(chats); }
  ...
  if (data.startsWith(dstTCh) || data.startsWith(dstTGr)) {
    ...
    const s2 = gs(uid);
    await edit(`⚙️ *إعدادات التحويل*\n\n📌 *الوجهة:* ${item.name}\n\nاضبط الخيارات ثم احفظ:`, settingsKb(s2.blockLinks, s2.noForward));
    return true;
  }
  ...
  if (data === "fw_set_lnk") {
    ...
    const s = gs(uid);
    await edit("⚙️ *إعدادات التحويل*", settingsKb(s.blockLinks, s.noForward));
    return true;
  }
  ...
  if (data.startsWith("fw_rl_")) {
    ...
    const chats = loadChats();
    const allItems = [...chats.groups, ...chats.channels];
    const srcNames = rule.sources.map((id) => allItems.find((x) => x.id === id)?.name || id.split("@")[0]).join("\n  • ");
    const dstName = allItems.find((x) => x.id === rule.destination)?.name || rule.destination?.split("@")[0];
    await edit(
...
export async function handleForwardText(msg) {
  ...
  if (sess.step === "dst_phone") {
    ...
    const s = gs(uid);
    await send(`✅ تم تحديد الشخص: +${digits}\n\nاضبط إعدادات التحويل:`, settingsKb(s.blockLinks, s.noForward));
    return true;
  }
  ...
  if (sess.step === "add_ch_jid") {
    ...
    if (input.includes("whatsapp.com/channel/")) {
      ...
      try {
        ...
        if (!meta?.id) throw new Error("no meta");
        const name = (typeof meta.name === "string" && meta.name) ? meta.name : ("📺 " + meta.id.split("@")[0]);
        const chats = loadChats();
        if (!chats.channels.find((c) => c.id === meta.id)) {
          chats.channels.push({ id: meta.id, name });
          saveChats(chats);
          if (_sock?.subscribeNewsletterUpdates) _sock.subscribeNewsletterUpdates(meta.id).catch(() => {});
        }
    ...
    if (!chats.channels.find((c) => c.id === jid)) {
      chats.channels.push({ id: jid, name: label + digits });
      saveChats(chats);
...
export async function subscribeForwardChannels(sock) {
  ...
  try {
    ...
    for (const ch of chats.channels) {
      if (ch.id?.endsWith("@newsletter")) {
        await sock.subscribeNewsletterUpdates(ch.id).catch(() => {});
      }
    }
  } catch {}
}
